    ...
    try {
        ...
        if (result?.imageqris) {
            try {
                ...
                console.warn('⚠️ Failed to download QR image:', imgErr.message);
            }
        }
        ...
        await pool.execute(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [partner_reff, customerId, body.customer_name, body.amount, expired,
            body.customer_phone || null, body.customer_email, result?.imageqris || null,
            qrisImageBuffer, JSON.stringify(result), mysqlNow(), isSuccess ? 'PENDING' : 'FAILED']);
...
async function updateOrderStatusFromCallback(partner_reff, paymentStatus) {
    try {
        ...
        if (result.affectedRows > 0) {
            console.log(`✅ Order status updated for partner_reff: ${partner_reff}`);
...
Kami akan segera mencari driver terdekat.
...
app.post('/sync-all-orders', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, message: `Synced ${updated} orders`, total_found: orders.length, updated });
    } catch (error) {
...
app.get('/orders', async (req, res) => {
    try {
        ...
        res.json({ success: true, count: results.length, data: results, pagination: { limit: parsedLimit, offset: parsedOffset } });
    } catch (err) {
...
app.put('/orders/:order_id', async (req, res) => {
    ...
    try {
        ...
        if (body.driver_id && body.driver_name) {
            ...
            if (orders.length > 0) await sendDriverAssignedNotification(orders[0]);
        }
...
// ============================================================
// FUNGSI NOTIFIKASI DRIVER ASSIGNED (SEDERHANA)
// ============================================================
...
app.post('/driver-confirmation', async (req, res) => {
    ...
    const result = await sendWhatsAppTemplate(driver_phone, CONFIG.templateDriverConfirmation, variables);

    res.json({ success: result.success, order_id, ...result });
});

// ============================================================
// ENDPOINT: WEBHOOK WHATSAPP (YANG PALING PENTING - SUDAH DIPERBAIKI)
// ============================================================
app.post('/webhook/whatsapp', express.urlencoded({ extended: true }), async (req, res) => {
    ...
    if (foundOrderId && foundConfirmation) {
        if (message === 'ACCEPT' || message === 'TERIMA' || message === 'SETUJU' || message === 'YES' || message === 'YA') {
            ...
            // ✅ INI FUNGSI UTAMA YANG DIPERBAIKI - akan AUTO CREATE order jika belum ada
...
async function sendOrderDetailsToDriver(orderId, confirmation) {
    ...
    try {
        ...
        // ✅ FIX: Jika order belum ada, BUAT dari data confirmation
        if (orders.length === 0) {
        ...
        await sendWhatsAppTemplate(confirmation.driver_phone, CONFIG.templateDriverOrderAccepted, {
            "1": confirmation.driver_name, "2": order.customer_name, "3": order.customer_phone,
            "4": orderId, "5": "Detail pesanan: " + (order.order_note || '-'), "6": formatRupiah(order.total_price)
        });

        // Kirim notifikasi ke customer
        await sendCustomerNotification(order, confirmation);
    } catch (error) {
...
// ============================================================
async function sendCustomerNotification(order, confirmation) {
...
Driver akan segera menghubungi Anda. 🙏`;
    ...
    await sendWhatsAppFreeForm(customerPhone, message);
}

// ============================================================
// FUNGSI NOTIFIKASI KE CUSTOMER (TEMPLATE VERSION)
// ============================================================
...
app.get('/drivers', async (req, res) => {
    ...
    const { bearer_token, detail = 'true' } = req.query;
    const finalToken = bearer_token || HARDCODED_TOKEN;

    res.writeHead(200, { 'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', 'Connection': 'keep-alive', 'Access-Control-Allow-Origin': '*' });
    const sendEvent = (eventName, data) => { res.write(`event: ${eventName}\ndata: ${JSON.stringify(data)}\n\n`); };

    sendEvent('start', { message: 'Memulai pengambilan data driver' });
    // SSE implementation simplified for brevity - keep your existing working code
    sendEvent('complete', { total_drivers: 0 });
